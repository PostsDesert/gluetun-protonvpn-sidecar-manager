...
### 2. Power Governor (Linux Host)
By default, Linux often uses the `powersave` governor, which can cause latency spikes as the CPU clocks up and down. For maximum VPN throughput, switch to the `performance` governor on the host machine:

```bash
echo performance | sudo tee /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor
```

To make this persistent across reboots, install `cpufrequtils`:
```bash
...
### 3. Network Tuning (Applied Automatically)
